...
CREATE UNIQUE INDEX `locales_name_unique` ON `locales` (`name`);--> statement-breakpoint
...
CREATE TABLE `media` (
	...
	FOREIGN KEY (`created_by`) REFERENCES `users`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`updated_by`) REFERENCES `users`(`id`) ON UPDATE no action ON DELETE cascade,
	FOREIGN KEY (`media_mime_type_id`) REFERENCES `mime_types`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE UNIQUE INDEX `media_storage_key_unique` ON `media` (`storage_key`);--> statement-breakpoint
CREATE INDEX `idx_media_name` ON `media` (`name`);--> statement-breakpoint
